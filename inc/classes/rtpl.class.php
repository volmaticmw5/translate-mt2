...
class RainTPL{
		...
		// -------------------------
				 ...
				 * @var unknown_type
				 */
				static $black_list = array( '\$this', 'raintpl::', 'self::', '_SESSION', '_SERVER', '_ENV',  'eval', 'exec', 'unlink', 'rmdir', 'system' );
		...
		const CACHE_EXPIRE_TIME = 86400; // default cache expire time = 3600, hour
		...
		*/
		 ...
		 */
		 ...
		 /***** Modified by Think, added replace_dir *******/
		private function path_replace( $html, $tpl_basedir ){

				if( self::$path_replace ){

						// reduce the path
						$path = preg_replace('/\w+\/\.\.\//', '', self::$base_url . self::$replace_dir  );

						$exp = $sub = array();

						if( in_array( "img", self::$path_replace_list ) ){
								$exp = array( '/<img(.*?)src=(?:")(http|https)\:\/\/([^"]+?)(?:")/i', '/<img(.*?)src=(?:")([^"]+?)#(?:")/i', '/<img(.*?)src="(.*?)"/', '/<img(.*?)src=(?:\@)([^"]+?)(?:\@)/i' );
								$sub = array( '<img$1src=@$2://$3@', '<img$1src=@$2@', '<img$1src="' . self::$base_url . self::$replace_dir  . '$2"', '<img$1src="$2"' );
						}
						...
						if( in_array( "script", self::$path_replace_list ) ){
								...
								$sub = array_merge( $sub , array( '<script$1src=@$2://$3@', '<script$1src=@$2@', '<script$1src="' . self::$base_url . self::$replace_dir  . '$2"', '<script$1src="$2"' ) );
						}
						...
						if( in_array( "link", self::$path_replace_list ) ){
								...
								$sub = array_merge( $sub , array( '<link$1href=@$2://$3@', '<link$1href=@$2@' , '<link$1href="' . self::$base_url . self::$replace_dir  . '$2"', '<link$1href="$2"' ) );
						}
						...
						if( in_array( "input", self::$path_replace_list ) ){
								...
								$sub = array_merge( $sub , array( '<input$1src=@$2://$3@', '<input$1src=@$2@', '<input$1src="' . self::$base_url . self::$replace_dir  . '$2"', '<input$1src="$2"' ) );
						}
